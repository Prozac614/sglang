#!/usr/bin/env python3
"""
Test 2: Full LoRA test (both high and low noise transformers)

This test:
1. Downloads LoRA weights from Cseti/wan2.2-14B-Arcane_Jinx-lora-v1
2. Sets LoRA for BOTH transformers (target="all")
3. Generates a video with full LoRA applied
4. Compares with baseline to show LoRA effect

Usage:
    python 2_test_full_lora.py [--host HOST] [--port PORT]
"""
...
def download_lora_files():
    """Download high and low noise LoRA files from HuggingFace."""
...
def main():
    parser = argparse.ArgumentParser(description="Full LoRA test (both transformers)")
    ...
    parser.add_argument(
        ...
        help="Prompt for generation (should match LoRA style)",
    )
    ...
    print("\n" + "=" * 60)
    print("TEST 2: FULL LoRA (Both Transformers)")
    print("=" * 60)
    print(f"Server: {base_url}")
    print(f"LoRA: {LORA_REPO}")

    # Step 1: Download LoRA files
    print("\n" + "-" * 60)
    print("Step 1: Downloading LoRA files")
    print("-" * 60)
    high_path, low_path = download_lora_files()

    # Step 2: Set LoRA for high-noise transformer
    print("\n" + "-" * 60)
    print("Step 2: Setting LoRA for high-noise transformer")
    print("-" * 60)
    set_lora(
        base_url=base_url,
        lora_nickname="arcane_high",
        lora_path=high_path,
        target="transformer",
    )

    # Step 3: Set LoRA for low-noise transformer
    print("\n" + "-" * 60)
    print("Step 3: Setting LoRA for low-noise transformer")
    print("-" * 60)
    set_lora(
        base_url=base_url,
        lora_nickname="arcane_low",
        lora_path=low_path,
        target="transformer_2",
    )

    # Step 4: Generate video with full LoRA
    print("\n" + "-" * 60)
    print("Step 4: Generating video with full LoRA")
    print("-" * 60)
    generate_video(
        base_url=base_url,
        prompt=args.prompt,
        ...
        test_name="2_full_lora_both_transformers",
    )

    print("\n" + "=" * 60)
    print("TEST 2 COMPLETED: Full LoRA video generated")
    print("=" * 60)
    print("\nCompare with baseline to see LoRA effect!")
    print("The video should show Arcane/Jinx style characteristics.")
    print("\nNext: Run 3_test_high_noise_only.py to test partial LoRA")
