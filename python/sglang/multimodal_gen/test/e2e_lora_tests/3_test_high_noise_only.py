#!/usr/bin/env python3
"""
Test 3: High-noise transformer LoRA only

This test:
1. Unmerges any existing LoRA
2. Sets LoRA ONLY for the high-noise transformer (target="transformer")
3. Generates a video to see the effect of partial LoRA

The high-noise transformer handles early denoising steps, affecting
overall structure and composition.

Usage:
    python 3_test_high_noise_only.py [--host HOST] [--port PORT]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="High-noise transformer LoRA only test"
    )
    ...
    print("\n" + "=" * 60)
    print("TEST 3: HIGH-NOISE TRANSFORMER LoRA ONLY")
    print("=" * 60)
    print(f"Server: {base_url}")
    print("\nThis tests LoRA applied ONLY to the high-noise transformer.")
    print("The high-noise transformer affects early denoising steps,")
    print("controlling overall structure and composition.")
    ...
    print("-" * 60)
    high_path = download_lora_files()

    # Step 3: Set LoRA for high-noise transformer ONLY
    print("\n" + "-" * 60)
    print("Step 3: Setting LoRA for HIGH-NOISE transformer only")
    print("-" * 60)
    set_lora(
        base_url=base_url,
        lora_nickname="arcane_high_only",
        lora_path=high_path,
        target="transformer",  # ONLY high-noise transformer
    )

    # Step 4: Generate video
    print("\n" + "-" * 60)
    print("Step 4: Generating video with HIGH-NOISE LoRA only")
    print("-" * 60)
    generate_video(
        base_url=base_url,
        prompt=args.prompt,
        ...
        test_name="3_high_noise_lora_only",
    )

    print("\n" + "=" * 60)
    print("TEST 3 COMPLETED: High-noise LoRA only video generated")
    print("=" * 60)
    print("\nCompare with:")
    print("  - Baseline (no LoRA): structural similarity")
    print("  - Full LoRA: see effect of missing low-noise LoRA")
    print("\nNext: Run 4_test_low_noise_only.py to test low-noise LoRA only")
